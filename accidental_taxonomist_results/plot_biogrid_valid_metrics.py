"""Generate SS vs NS violin plots for VALID (validation) metrics.

Mirrors plot_biogrid_test_metrics.py but extracts the last VALID snapshot
from each run's metrics.log instead of the last TEST snapshot.
"""
import os
import re
import json
...
def summarize_valid_metrics(run_dir: str) -> Tuple[int, bool, Dict[str, float]]:
    """Return (seed, matching_orgs, last_VALID_metrics_dict) for a run directory."""
    ...
    if parsed['VALID']:
        last_valid = parsed['VALID'][-1]

    return seed, bool(matching_orgs), last_valid


def build_dataframe_from_runs(run_dirs: List[str]) -> pd.DataFrame:
    rows: List[Dict[str, object]] = []
    for idx, rd in enumerate(run_dirs):
        seed, matching_orgs, metrics = summarize_valid_metrics(rd)
...
def plot_ss_ns_violin_for_metric(df: pd.DataFrame, metric_name: str, out_dir: str) -> None:
    ...
    fname_base = f"biogrid_valid_{sanitize_metric_name(display_label)}"
    ...
    with open(os.path.join(out_dir, f'{fname_base}_tests.json'), 'w') as f:
    ...
    with open(os.path.join(out_dir, f'{fname_base}_summary.json'), 'w') as f:
...
def main():
    parser = argparse.ArgumentParser(description='Aggregate VALID metrics and plot SS vs NS per metric.')
    parser.add_argument(
        '--runs-root',
        type=str,
        default=os.path.join('biogrid_species_experiment'),
        help='Root directory containing biogrid_*_seed* run folders'
    )
    parser.add_argument(
        '--out-dir',
        type=str,
        default=os.path.join('..', '..', 'figures'),
        help='Directory to store per-metric plots and stats JSONs'
    )
    ...
    print(f'Saved per-metric validation plots under: {args.out_dir}')
